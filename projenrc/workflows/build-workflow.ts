...
if [ -f repo.patch ]; then
  ...
  echo "" | tee -a debug-output.txt
  echo "=== AZ-TAGGER NESTED STACK CONTENT ===" | tee -a debug-output.txt
  if [ -f "cdk.out/multiazworkshopaztaggerD1FA9152.nested.template.json" ]; then
    echo "Contents of az-tagger nested stack template (multiazworkshopaztaggerD1FA9152.nested.template.json):" | tee -a debug-output.txt
    cat cdk.out/multiazworkshopaztaggerD1FA9152.nested.template.json | tee -a debug-output.txt
  else
    echo "az-tagger nested stack template file not found at cdk.out/multiazworkshopaztaggerD1FA9152.nested.template.json" | tee -a debug-output.txt
  fi
  echo "" | tee -a debug-output.txt
